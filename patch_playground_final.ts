...
content = content.replace(
  ...
  "{ id: 'memory-mem-1', content: 'blue', metadata: { label: 'favorite-color', userId: 'user-1' }, tags: [], relationships: [] }"
);
...
content = content.replace(
  ...
  "console.log('Learning generated promotions:', promotions.knowledgePromotions?.length || 0);"
);

content = content.replace(
  "console.log('Noise Learning promotions (should be 0 or rejected):', rejectedPromotions.proposedNodes?.length || 0);",
  "console.log('Noise Learning promotions (should be 0 or rejected):', rejectedPromotions.knowledgePromotions?.length || 0);"
);
