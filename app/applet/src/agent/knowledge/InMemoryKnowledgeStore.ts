...
export class InMemoryKnowledgeStore implements KnowledgeStore {
  ...
  async createRecord(recordData: Omit<KnowledgeRecord, 'createdAt' | 'updatedAt' | 'version'>): Promise<KnowledgeRecord> {
    const now = Date.now();
  ...
  async createCollection(collectionData: Omit<KnowledgeCollection, 'createdAt' | 'updatedAt'>): Promise<KnowledgeCollection> {
    const now = Date.now();
