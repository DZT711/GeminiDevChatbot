import { ModelId, Provider } from './types';
import { modelQueueManager } from './modelQueueManager';
import { ChatSession, Message, Attachment } from './chatSessionManager';
...
class GeminiService {
  ...
  public attachmentToPart(attachment: Attachment) {
    ...
    return { text: `[File Attachment: ${attachment.name}]${attachment.content || ""}` };
  }
  ...
  async generateResponse(
    ...
    if (true) {
      ...
      const activeSkills = allSkills.filter(s => activeSkillIds.includes(s.id));
            ...
            if (config.session && config.isAutoCompact) {
...
Always provide full, runnable code blocks where applicable. Use Markdown for formatting.`;
      ...
      if (config.customInstructions) {
        systemPrompt += `User Custom Personalization/Instructions:${config.customInstructions}`;
      }

          let attempts = 0;
          const isHybrid = config.model === ModelId.HYBRID;
          const startModel = isHybrid ? modelQueueManager.getCurrentModel() : (config.model || modelQueueManager.getCurrentModel());
          let startIndex = modelQueueManager.getQueue().indexOf(startModel);
          if (startIndex === -1) startIndex = 0;
          
          while (attempts < modelQueueManager.getQueue().length) {
            if (config.signal?.aborted) throw new Error("Operation aborted");
            const model = modelQueueManager.getQueue()[(startIndex + attempts) % modelQueueManager.getQueue().length];
            ...
            try {
                  ...
                  for (const call of functionCalls) {
                    ...
                    if (call.name === 'analyze_github_repo') {
                      ...
                      if (!url) {
                        ...
                        try {
                          ...
                          if (info) {
                            responsePayload = {
                              status: "success",
                              summary: `Repository scan complete.${githubService.formatRepoSummary(info)}`,
                              files: info.topFiles
                            };
                          } else {
                      ...
                      try {
                        ...
                        config.onEvent?.({ type: 'knowledge-interaction', payload: {
                            action: 'propose',
                            source: 'model',
              ...
              if (attempts < modelQueueManager.getQueue().length - 1) {
                ...
                const errorMsg = `\`\`\`ansi\x1b[31m[NEURAL_ERROR: ${errorCode}]\x1b[0m ${error.message}\x1b[33m[FALLBACK_ACTIVATED]\x1b[0m Rerouting payload to node: ${nextModelId.split('/').pop()}\`\`\``;
                onChunk?.(errorMsg);
                
                modelQueueManager.rotateModel();
                attempts++;
                continue;
              } else {
                throw error;
...
- For updates (UPDATE) or deletions (DELETE), the change is queued as a PENDING proposal needing administrator approval before taking effect. Inform the developer that a pending admin review proposal has been queued.`;
      ...
      let attempts = 0;
      let startIndex = modelQueueManager.getQueue().indexOf(config.model || modelQueueManager.getCurrentModel());
      if (startIndex === -1) startIndex = 0;
